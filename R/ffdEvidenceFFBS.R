...
#' @param S0 prior covariance structure among voxels within every cluster at \code{t=0}. \code{S0=1} is the default value when no prior information is available and defines an \eqn{q\times q} identity matrix. For the case of available prior information, \eqn{S0} can be defined as an \code{q\times q} matrix, where \eqn{q} is the common number of voxels in every cluster.
