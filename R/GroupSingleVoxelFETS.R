...
#' @param S0 prior covariance structure between pair of voxels within every cluster at \code{t=0}. \code{S0=1} is the default value when no prior information is available and defines an \times{q \times q} identity matrix. For the case of available prior information, \code{S0} can be defined as an \eqn{q \times q} matrix, where \eqn{q} is the common number of voxels in every cluster.
