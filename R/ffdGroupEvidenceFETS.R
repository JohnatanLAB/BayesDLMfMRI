...
#' @param r1 positive integer number that defines the distance from every voxel with its most distant neighbor. This value determines the size of the cluster. The users can set a range of different \code{r1} values: \eqn{r1 = 0, 1, 2, 3, 4}, which leads to \eqn{q = 1, 7, 19, 27, 33}, where \eqn{q} is the size of the cluster.
...
#' @return It returns a list of \eqn{2 \times p} elements, where \eqn{p} is the number of covariates, and 2 is the number 
